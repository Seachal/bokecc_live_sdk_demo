...
public class VoteSummaryAdapter extends RecyclerView.Adapter<VoteSummaryAdapter.VoteViewHolder> {
    ...
    public void onBindViewHolder(VoteViewHolder holder, int position) {
        ...
        holder.mWrongProgressBar.setMax(100);
        holder.mRightProgressBar.setMax(100);

        if (voteSingle.isCorrect()) {
            holder.mWrongProgressBar.setVisibility(View.GONE);
            holder.mRightProgressBar.setVisibility(View.VISIBLE);
            holder.mRightProgressBar.setProgress((int)(Float.parseFloat(voteSingle.getPercent())));
        } else {
            holder.mWrongProgressBar.setVisibility(View.VISIBLE);
            holder.mRightProgressBar.setVisibility(View.GONE);
            holder.mWrongProgressBar.setProgress((int)(Float.parseFloat(voteSingle.getPercent())));
        }
