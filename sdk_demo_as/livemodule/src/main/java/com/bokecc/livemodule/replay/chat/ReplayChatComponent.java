...
public class ReplayChatComponent extends RelativeLayout implements DWReplayChatListener,
    ...
    public void onChatMessage(TreeSet<ReplayChatMsg> replayChatMsgs) {
        ...
        Log.e("###","chatEntities 0 time:"+chatEntities.get(0).getTime());
        if (mChatEntities == null){
        mChatEntities = chatEntities;
        }else{
            mChatEntities.addAll(chatEntities);
        }
        Log.e("###","onChatMessage:"+mChatEntities.size());
        Collections.sort(mChatEntities, new Comparator<ChatEntity>() {
            @Override
            public int compare(ChatEntity o1, ChatEntity o2) {
                if (TextUtils.isEmpty(o1.getTime())){
                    return 0;
                }
                if (TextUtils.isEmpty(o2.getTime())){
                    return 0;
                }
