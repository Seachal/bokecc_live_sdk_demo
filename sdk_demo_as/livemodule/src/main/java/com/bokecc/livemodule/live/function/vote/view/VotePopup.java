    ...
    public void onVoteResult(JSONObject jsonObject) {
        ...
        try {
            ...
            for (int i = 0; i < statisics.length(); i++) {
                VoteSingleStatisics v = new VoteSingleStatisics(statisics.getJSONObject(i));
                //判断自己的选项是否正确
                boolean isCorrect = false;
                if (isOptionIsRight(voteType,v)){
                        isCorrect = true;
//                    if (checkOptionIsCorrect(voteType,v)){
//                        isCorrect = true;
//                    }
                }
                v.setCorrect(isCorrect);
                voteStatisices.add(v);
            }
    ...
    private boolean checkOptionIsCorrect(int voteType, VoteSingleStatisics v) {
        if (voteType==1){
            for (int i = 0; i < selectOptions.size(); i++) {
                if (selectOptions.get(i)!=null){
                    try{
                        if (Integer.parseInt(selectOptions.get(i))==v.getOption()){
                            return true;
                        }
                    }catch (Exception e){
                         e.printStackTrace();
            ...
            return selectOption==v.getOption();
        }
        return false;
    }

    private boolean isOptionIsRight(int voteType, VoteSingleStatisics v) {
        if (voteType==1){
            //多选
            if (correntOptions!=null){
                for (int a = 0; a < correntOptions.length; a++) {
                    try{
                        if (correntOptions[a]!=null&&Integer.parseInt(correntOptions[a])==v.getOption()){
                            return true;
                        }
                    }catch (Exception e){
                        e.printStackTrace();
